...
- [第10章 数据仓库和Web](#%e7%ac%ac10%e7%ab%a0-%e6%95%b0%e6%8d%ae%e4%bb%93%e5%ba%93%e5%92%8cweb)
- [第11章 非结构化数据和数据仓库](#%e7%ac%ac11%e7%ab%a0-%e9%9d%9e%e7%bb%93%e6%9e%84%e5%8c%96%e6%95%b0%e6%8d%ae%e5%92%8c%e6%95%b0%e6%8d%ae%e4%bb%93%e5%ba%93)
- [第12章 大型数据仓库](#%e7%ac%ac12%e7%ab%a0-%e5%a4%a7%e5%9e%8b%e6%95%b0%e6%8d%ae%e4%bb%93%e5%ba%93)
- [第13章 关系模型和多维模型数据库设计基础](#%e7%ac%ac13%e7%ab%a0-%e5%85%b3%e7%b3%bb%e6%a8%a1%e5%9e%8b%e5%92%8c%e5%a4%9a%e7%bb%b4%e6%a8%a1%e5%9e%8b%e6%95%b0%e6%8d%ae%e5%ba%93%e8%ae%be%e8%ae%a1%e5%9f%ba%e7%a1%80)
  - [13.1 关系模型](#131-%e5%85%b3%e7%b3%bb%e6%a8%a1%e5%9e%8b)
  - [13.2 多维模型](#132-%e5%a4%9a%e7%bb%b4%e6%a8%a1%e5%9e%8b)
  - [13.3 雪花结构](#133-%e9%9b%aa%e8%8a%b1%e7%bb%93%e6%9e%84)
  - [13.4 两种模型的区别](#134-%e4%b8%a4%e7%a7%8d%e6%a8%a1%e5%9e%8b%e7%9a%84%e5%8c%ba%e5%88%ab)
  - [13.5 独立数据集市](#135-%e7%8b%ac%e7%ab%8b%e6%95%b0%e6%8d%ae%e9%9b%86%e5%b8%82)
  - [13.6 建立独立数据集市](#136-%e5%bb%ba%e7%ab%8b%e7%8b%ac%e7%ab%8b%e6%95%b0%e6%8d%ae%e9%9b%86%e5%b8%82)
- [第14章 数据仓库高级话题](#%e7%ac%ac14%e7%ab%a0-%e6%95%b0%e6%8d%ae%e4%bb%93%e5%ba%93%e9%ab%98%e7%ba%a7%e8%af%9d%e9%a2%98)
- [第16章 数据仓库和ODS](#%e7%ac%ac16%e7%ab%a0-%e6%95%b0%e6%8d%ae%e4%bb%93%e5%ba%93%e5%92%8cods)

# 第1章 决策支持系统的发展
决策支持系统(Decision Support System,DSS) p1

信息管理系统(Information Management System,IMS) p6

集成数据库管理系统(Integrated Database Management System, IDMS) p6
...
粒度管理器（GM），读入点击流数据并做如下工作：p183

- 清除无关数据
- 根据多个相关的点击流日志记录生成一条记录
- 清楚错误数据
- 对在Web环境中独一无二的数据，尤其是那些需要用于同其他企业数据进行
- 集成的关键数据进行转换
- 对数据进行汇总
- 对数据进行聚集

将数据从Web转移到数据仓库涉及以下这些步骤：p184

- Web数据收集到日志
- 日志数据在通过粒度管理器时进行处理
- 粒度管理器将提炼后的数据传递给数据仓库

操作型数据存储（ODS）：是一个混合结构体，既具有数据仓库的某些特征，又兼具操作型系统的一些特征。一方面，ODS包含了集成数据，能支持决策支持系统的处理。另一方面，ODS又支持高性能的事务处理。p184
- ODS中存放的都是解释性数据。从数据仓库中读取出来的数据，经分析后转换为“概要”数据或概要记录。概要记录存放在ODS中。

![](BuildingTheDataWarehouse41.jpg)

粒度管理器负责从Web向数据仓库装载数据，数据装载的周期是以天甚至小时来计的，这取决于Web上的平均数据流量。数据仓库中的数据以每月或每季度为周期装载到海量存储设备。通过这种处理方式，体系结构中的每一级都不会有多到不可管理的数据。
- 通常，Web环境可能保存一天的数据，数据仓库可能保存一年的数据，而海量存储设备则可能存有长达十年的数据。

![](BuildingTheDataWarehouse42.jpg)

ODS可以实现毫秒级的响应时间，而数据仓库则不能。数据仓库中存放的是详细的历史信息，而ODS与此不同，它存放的是概要数据。p190

# 第11章 非结构化数据和数据仓库
非结构化数据大致分为两类——通信和文档。通信相对较短且分布有限，而且趋向于一个较短的生命周期。文档则是面向更广大的读者，并且比通信要大很多。文档的生命周期也要比通信长很多。通信和文档的基本形式都是文本。p192
...
![](BuildingTheDataWarehouse43.jpg)

适用于两个环境 p206
- 标识符用来专门标识一条记录的数据。典型的标识符有社会保险号，职工号和驾照号。标识符对它所标识的内容是特定的。一旦标识确定，很大程度上记录的标识被适当地确定。
- 紧密标识符是指存在高概率的标识符。紧密标识符包括名字、地址和其他标识数据。标识符和紧密标识符的区别在于确定性。如果根据社会保险号来确定一个人，那么的确是这个人的概率很大。但是用紧密标识符未必如此，比如名字。
- 当出现概率匹配的时候，紧密标识符为概率匹配中不同的数据提供联接。

![](BuildingTheDataWarehouse44.jpg)

![](BuildingTheDataWarehouse45.jpg)

# 第12章 大型数据仓库
历史数据-细节数据-多种数据=大量数据 p209

![](BuildingTheDataWarehouse46.jpg)

# 第13章 关系模型和多维模型数据库设计基础
## 13.1 关系模型
p223

![](BuildingTheDataWarehouse47.jpg)

有几种不同的数据表，通过一系列外键关键字互相关联。外键关键字关联是指在两张数据表中存在同一数据单元的基本关联。

![](BuildingTheDataWarehouse48.jpg)

关系型数据以一种称为“标准化”的形式存在。数据标准化是指数据库设计会使数据分解成非常低的粒度级。标准化的数据以一种孤立模式存在，这种情况下对数据表里的数据关系要求很严格。当进行标准化的时候，表中的数据只能与这张表里的其他数据关联。

## 13.2 多维模型
p223

建立数据仓库的另一种数据库设计方法通常认为是多维模型方法。多维模型方法也叫做星形连接。

![](BuildingTheDataWarehouse49.jpg)

图13-5表明在星形连接的中心是一张事实表。事实表是包含大量数据值的一种结构。事实表的周围是维表，用来描述事实表的某个重要方面。维表里的数据量要比事实表里的少。

## 13.3 雪花结构
p224

星形连接只包含一张事实表。但是在数据库设计中要创建一种雪花结构的复合结构需要多张事实表结合。

![](BuildingTheDataWarehouse50.jpg)

在雪花结构中，不同的事实表通过共享一个或多个公共维表连接起来。有时称这些共享的维表为一致维表。

## 13.4 两种模型的区别
关系模型适合于大范围数据（如一个企业模型），而多维模型适用于小范围数据（如一个部门或甚至一个子部门）p225

关系模型适合数据的间接访问而多维模型利于数据的直接访问。p226

关系模型的另一优点是具有适度变化的能力。关系模型设计以间接方式使用。也就是说，数据仓库的直接用户访问的是由关系模型转化而来的数据而不是关系模型本身的数据。p227

星形连接即多维方法却不具有适度变化的能力这一特征。多维数据库设计是很脆弱的，是很多处理请求聚集在一起的结果。当处理请求变化时，多维数据库的设计未必能够适度地变化。数据一旦以多维数据库形式建立，要想再改动它就很难了。p228

关系模型是数据仓库设计的最佳基础。p228

## 13.5 独立数据集市
多维模型的另一个特点是通过一种称作独立数据集市方法联合起来。数据集市是用来表示服务一组特定群体（如财会部门或金融部门）的分析需求的一种数据结构。p229

![](BuildingTheDataWarehouse51.jpg)

## 13.6 建立独立数据集市
p232

![](BuildingTheDataWarehouse52.jpg)

![](BuildingTheDataWarehouse53.jpg)

![](BuildingTheDataWarehouse54.jpg)

![](BuildingTheDataWarehouse55.jpg)

![](BuildingTheDataWarehouse56.jpg)

![](BuildingTheDataWarehouse57.jpg)

# 第14章 数据仓库高级话题
企业信息工厂（CIF）p233

数据仓库和数据模型 p233
- 高层数据模型显示出数据仓库的不同主题域是如何分割的。典型的高层主题域是客户，产品，装运情况，订单，部件数目等。
- 中层数据模型确定键、属性和数据仓库的其他细节。中层数据模型使高层数据模型“有血有肉”。
- 底层数据模型用来进行数据仓库的物理设计。在这一层上会进行分区，对DBMS定义外键关系，定义索引以及完成其他物理方面的设计。

![](BuildingTheDataWarehouse58.jpg)

当对资源的竞争成为一个问题时，最好能够分析一种特殊形式的数据仓库。这种形式称作探查型数据仓库或数据挖掘型数据仓库。p235
- 探查型数据仓库为处理繁重的统计分析提供基础。
- 探查型数据仓库与数据挖掘型数据仓库的一个区别是这样的：探查型数据仓库面向广度进行优化，数据挖掘型数据仓库面向深度进行优化。

![](BuildingTheDataWarehouse59.jpg)

为了支持数据的及时访问，网站访问ODS，而不是数据仓库。p244
- 创建概要记录并且放在ODS中就很有意义了。
...
# 第16章 数据仓库和ODS
互补的结构 p263
- ODS在设计上能够满足响应时间为2-3秒的事务。
- ODS中的记录一般称为“概要记录”。
- ODS中概要记录的意义在于能够快速访问。

![](BuildingTheDataWarehouse60.jpg)

对ODS处理日进行分片 p267
- 通过时间分片，ODS可以支持多种处理，可以保障每天的高峰时间能够高效运作。

![](BuildingTheDataWarehouse61.jpg)